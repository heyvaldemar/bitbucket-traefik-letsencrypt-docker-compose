#!/usr/bin/env bash
# bitbucket-restore-application-data.sh [backup-file-name]
#
# Replaces Bitbucket's application data with one of the archives the backups
# service wrote.
#
#   ./bitbucket-restore-application-data.sh               list and ask
#   ./bitbucket-restore-application-data.sh <file-name>   restore that one
#
...
if ! docker exec "$BKP" sh -c "set -eu
  ...
  echo "error: the restore failed part-way; $DATA may be incomplete. Restore another archive before using Bitbucket." >&2
  exit 1
fi
