#!/bin/bash
# End-to-end tests for the bitbucket-traefik-letsencrypt-docker-compose backup + restore flow.
#
...
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-bitbucket}"
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-bitbucket-traefik-letsencrypt-docker-compose.yml}"
...
DB_NAME="${BITBUCKET_DB_NAME}"
DB_USER="${BITBUCKET_DB_USER}"
DB_HOST="postgres"
...
test_restore_roundtrip() {
  ...
  echo "  restoring the baseline with ./bitbucket-restore-database.sh"
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./bitbucket-restore-database.sh "$(basename "$baseline")" \
...
test_data_restore_roundtrip() {
  ...
  echo "  restoring $(basename "$newest") with ./bitbucket-restore-application-data.sh"
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./bitbucket-restore-application-data.sh "$(basename "$newest")" \
    || { fail "the shipped restore script failed"; return 1; }
