#!/usr/bin/env bash
# bitbucket-restore-database.sh [backup-file-name]
#
# Replaces the Bitbucket database with one of the dumps the backups service wrote.
#
#   ./bitbucket-restore-database.sh               list and ask
#   ./bitbucket-restore-database.sh <file-name>   restore that one
#
...
#
# Set COMPOSE_PROJECT_NAME if the stack was started with a -p other than bitbucket.
set -Eeuo pipefail

PROJECT="${COMPOSE_PROJECT_NAME:-bitbucket}"
APP_SERVICE="bitbucket"
...
DB_NAME="$(env_of BITBUCKET_DB_NAME)"; DB_USER="$(env_of BITBUCKET_DB_USER)"
...
if ! docker exec "$BKP" sh -c "(set -o pipefail) 2>/dev/null && set -o pipefail; set -eu
  ...
  echo "error: the restore failed part-way. The database may now be empty: restore another backup before using Bitbucket." >&2
  exit 1
fi
