...
Every release from v1.8.5 on carries three files made on GitHub's runner with a short-lived identity and no stored key: `bitbucket-traefik-letsencrypt-docker-compose-<tag>.tar.gz`, a `git archive` of exactly the tree the tag points at; `bitbucket-traefik-letsencrypt-docker-compose-<tag>.tar.gz.sigstore.json`, a keyless [Sigstore](https://www.sigstore.dev/) signature over it; and `bitbucket-traefik-letsencrypt-docker-compose-<tag>.intoto.jsonl`, [SLSA](https://slsa.dev/) build provenance from the SLSA generator. To check them with nothing from this repository trusted:

```bash
cosign verify-blob bitbucket-traefik-letsencrypt-docker-compose-<tag>.tar.gz \
  --bundle bitbucket-traefik-letsencrypt-docker-compose-<tag>.tar.gz.sigstore.json \
  --certificate-identity-regexp '^https://github.com/heyvaldemar/bitbucket-traefik-letsencrypt-docker-compose/' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com

slsa-verifier verify-artifact bitbucket-traefik-letsencrypt-docker-compose-<tag>.tar.gz \
  --provenance-path bitbucket-traefik-letsencrypt-docker-compose-<tag>.intoto.jsonl \
  --source-uri github.com/heyvaldemar/bitbucket-traefik-letsencrypt-docker-compose
```
...
The `backups` container runs a `pg_dump | gzip` + `tar.gz`-of-home → prune → sleep loop (defaults: 30-minute warm-up, 24-hour interval, 7-day retention). Restore with the interactive scripts (`chmod +x *.sh` once): `./bitbucket-restore-database.sh`, then `./bitbucket-restore-application-data.sh`. Each lists the backups and asks, or takes a file name as its argument; both read every path and credential from the running backups container, and CI runs both on every push. Repositories live in the home directory, so database and home must be restored as a matching pair.
